	...
	switch (method) {
		...
		case "POST":
			try {
				...
				const currentLargestId = Math.max(...bids.map((b) => b.id));
				
				// Simple blocker to keep record count down to prevent abuse.
				if (currentLargestId > 100) {
					return res.status(500).json({ error: 'Too many records already on file.' });
				}
				else {
